...
export function generateProductDescription(attributes: VisionAttributes): ProductDescription {
  ...
  if (colors.length > 0) description += ` in ${humanColorList(colors)}`;
  if (material) description += `, ${material}`;
  if (pattern) description += `, ${pattern}`;
  if (brandHints.length > 0) {
    const brandStyleNote = getBrandStyleNote(brandHints);
    if (brandStyleNote) description += ` — ${brandStyleNote}`;
    else description += ` — inspired by ${titleCase(brandHints[0])}`;
  }
  if (gender) description += `. ${titleCase(gender)}'s style.`; else description += '.';
  description += ` ${getStyleNote(category)}`;

  const queryParts: string[] = [];
  if (brandHints.length > 0) queryParts.push(brandHints[0]);
  queryParts.push(category);
  if (colors.length > 0) queryParts.push(colors[0]);
